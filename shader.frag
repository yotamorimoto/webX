...
void main() {
  ...
  vec3 col1 = palette(d1+time, vec3(0.5,0.5,0.5), vec3(0.5,0.5,0.5), vec3(0.5,0.5,0.5), vec3(0.0,0.10,0.20));

  // Determine length to point 2 & calculate color.
  float d2 = length(uv.yx - c2);
  vec3 col2 = palette(d2+time, vec3(0.5,0.5,0.5), vec3(0.5,0.5,0.5), vec3(0.5,0.5,0.5), vec3(0.0,0.10,0.20));
